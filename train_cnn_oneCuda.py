...
def main():
	...
	train_dataset = torchvision.datasets.ImageFolder(train_list, transform=xception_default_data_transforms['train'])
	val_dataset = torchvision.datasets.ImageFolder(val_list, transform=xception_default_data_transforms['val'])
	...
	for epoch in range(epoches):
		...
		for (image, labels) in train_loader:
			iter_loss = 0.0
			iter_corrects = 0.0
			image = image.to(torch.device('cuda:2'))
			labels = labels.to(torch.device('cuda:2'))
			optimizer.zero_grad()
			outputs = model(image)
...
if __name__ == '__main__':
	...
	parse.add_argument('--train_list', '-tl' , type=str, default = '/hdd/2017CS128/Research/FFc40-split2/train')
	parse.add_argument('--val_list', '-vl' , type=str, default = '/hdd/2017CS128/Research/FFc40-split2/validation')
	...
	parse.add_argument('--model_path', '-mp', type=str, default='/hdd/2017CS128/Research/Deepfake-Detection/output/xception-normal2/14_fs_c0_299.pkl')
	main()